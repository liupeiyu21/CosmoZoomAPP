...
export default function HomeScreen() {
  const [angles, setAngles] = useState(planets.map((_, i) => (Math.PI * 2 * i) / 8 - Math.PI / 2));
  const [selectedPlanet, setSelectedPlanet] = useState(null);
  const [planetPaused, setPlanetPaused] = useState(Array(planets.length).fill(false));
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={styles.container}>
        <Image source={require('../../assets/images/space_bg.png')} style={StyleSheet.absoluteFill} resizeMode="cover" />
